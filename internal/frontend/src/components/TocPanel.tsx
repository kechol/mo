...
export function TocPanel({ headings, activeHeadingId, onHeadingClick }: TocPanelProps) {
  ...
  const onResizeKeyDown = useCallback((e: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    // Panel is anchored on the right edge, so ArrowLeft expands it.
    if (e.key === "ArrowLeft") {
      e.preventDefault();
  ...
  return (
    ...
    >
      ...
      <div
        ...
        tabIndex={0}
        className="absolute top-0 left-0 w-1 h-full cursor-col-resize hover:bg-gh-border active:bg-gh-border focus:outline-none focus-visible:bg-gh-text-secondary transition-colors"
